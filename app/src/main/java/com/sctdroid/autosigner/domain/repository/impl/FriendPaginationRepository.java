 ...
 */
public class FriendPaginationRepository extends BaseListRepository implements PaginationRepository {
    private static final String TAG = FollowerListRepository.class.getSimpleName();
    FriendshipsAPI mFriendshipsAPI;
    public FriendPaginationRepository(Context context, String appKey, Oauth2AccessToken accessToken) {
        super(context, appKey, accessToken);
        mFriendshipsAPI = new FriendshipsAPI(context, appKey, accessToken);
    }

    @Override
    public void get(Bundle bundle, final PaginationRepository.OnGetListener listener) {
        int cursor = bundle.getInt("cursor");
        int count = bundle.getInt("count");
        mFriendshipsAPI.friends(Long.valueOf(mAccessToken.getUid()), count, cursor, false, new RequestListener() {
            @Override
